"""
Variant C2: Larger Stock Embedding (emb_dim=64) + 30 epochs + CosineAnnealingLR
Based on C_stock_emb, doubles the embedding dimension.
"""
...
def run():
    ...
    config['stock_emb_dim'] = 64  # doubled from C's 32
    config['output_dir'] = f'./model/60_{config["feature_num"]}/{VARIANT_NAME}'
    os.makedirs(config['output_dir'], exist_ok=True)

    print(f"[{VARIANT_NAME}] epochs={config['num_epochs']}, stock_emb_dim={config['stock_emb_dim']}")
    start = time.time()
    try:
