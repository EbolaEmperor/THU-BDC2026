"""
Shared prediction helper for all variants.
Runs prediction using the model in the given output_dir with the current config.
"""
import os, sys
import numpy as np
import pandas as pd
...
def run_prediction(config, variant_name, pred_date_str='2026-06-18'):
    """Run prediction and save results to config['output_dir']/prediction.txt"""
    ...
    with open(pred_path, 'w', encoding='utf-8') as f:
        f.write(f"=== {variant_name} Top-5 Predictions ===\n")
        f.write(f"Predict date: {pred_date_str} | Buy: 2026-06-22 | Sell: 2026-06-26\n\n")
    ...
    print(f"[{variant_name}] Predictions saved to {pred_path}")
    return top5
